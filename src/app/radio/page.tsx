            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {programs.map(prog => (
                <div key={prog.id} className="border border-border p-8 hover:border-foreground transition-colors flex flex-col justify-between">
                  <div>
                    <span className="text-[10px] font-bold uppercase tracking-widest text-red-600 mb-2 block">{prog.category}</span>
                    <h3 className="font-heading font-bold text-2xl mb-3">{prog.title}</h3>
                    <p className="text-muted-foreground text-sm font-medium leading-relaxed mb-6">{prog.description}</p>
                  </div>
